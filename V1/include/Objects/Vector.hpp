...
# ifdef __cplusplus
		...
		PrivateVector(size_t const size, size_t const capacity)
			: _data(nullptr), _size(size), _capacity(capacity) {};

#endif /* !CPLUSPLUS */
	};

	struct						PrivateVectorIterator
	{
		void					*_current;

# ifdef __cplusplus

		PrivateVectorIterator(void const * const ptr)
			: _current(const_cast<void *>(ptr)) {};
...
#  define  __SIZEOF_VECTOR__			sizeof(Objects::PrivateVector)
#  define  __SIZEOF_VECTOR_ITERATOR__	sizeof(Objects::PrivateVectorIterator)

# else

#  define  __SIZEOF_VECTOR__			sizeof(struct PrivateVector)
#  define  __SIZEOF_VECTOR_ITERATOR__	sizeof(struct PrivateVectorIterator)
...
# ifdef __cplusplus
	...
	class						Vector : private Objects::PrivateVector
	{
	public:
		class					Iterator : private Objects::PrivateVectorIterator
		{
		public:
			Iterator(void const * const ptr)
				: Objects::PrivateVectorIterator(ptr) {};

			Iterator(Objects::Vector<_Type>::Iterator const &copy)
				: Objects::PrivateVectorIterator(copy._current) {};
			...
			inline _Type		&operator*(void) const
				{
					...
					return *reinterpret_cast<_Type *>(this->_current);
				}
		...
		Vector(size_t const capacity = 0) : Objects::PrivateVector(0, capacity)
			{
				if (this->_capacity)
					this->_data = Objects::Memory::alloc(this->_capacity * sizeof(_Type));
			}

		Vector(Objects::Vector<_Type> const &copy)
			: Objects::PrivateVector(copy._size, copy._capacity)
			{
				if (this->_capacity)
				{
					this->_data = Objects::Memory::alloc(this->_capacity * sizeof(_Type));
		...
		Objects::Vector<_Type>	&operator=(Objects::Vector<_Type> const &other)
			{
				if (this != &other)
				{
					...
					if (this->_capacity != other._capacity)
					{
						...
						Objects::Memory::free(this->_data);
						this->_data = Objects::Memory::alloc(this->_capacity * sizeof(_Type));
					}
		...
		void										capacity(size_t const capacity)
			{
				if (capacity != this->_capacity)
				{
					if ((this->_capacity = capacity) > 0)
						this->_data = Objects::Memory::realloc(reinterpret_cast<void *>(this->_data),
															   sizeof(_Type) * this->_capacity);
					else
						Objects::Memory::free(this->_data);
		...
		inline Objects::Vector<_Type>::Iterator		end(void) const
			{
				return Objects::Vector<_Type>::Iterator((this->_data) ?
														(this->_data + (this->_size * sizeof(_Type))) : nullptr);
			}
		...
		void										push_back(_Type const &value)
			{
				...
				new(this->_data + (this->_size++ * sizeof(_Type))) _Type(value);
			}
		...
		void											insert(_Type const &value,
															   size_t const pos)
			{
				if (pos < this->_size)
				{
					...
					if (++this->_size >= this->_capacity)
						this->_capacity = this->_size;
					itData = data = reinterpret_cast<_Type *>(
						Objects::Memory::alloc(this->_capacity * sizeof(_Type)));
					while (it != itEnd)
					{
						if (i++ == pos)
							new (itData) _Type(value);
						else
							Objects::Memory::copy(reinterpret_cast<void *>(itData),
